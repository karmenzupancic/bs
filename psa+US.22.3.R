...
GT2year <- read_excel("C:/Users/karzup7386/Desktop/mesec 2/PCA/datadaily2.xlsx", sheet = 5)

yld   = na.omit(GT2year)
yield = as.matrix(yld[,2:11])
...
bond_yield_subset = as.matrix(bond_yield_data[,2:11]/100)
...
ggplot(residuals_data, aes(x = date)) +
  ...
  labs(title = "Residuals for Each Country",
       x = "Date",
       y = "Residuals") +
  scale_color_manual(values = c("DE" = "blue", "FR" = "green", "ES" = "red", "IT" = "purple", "GB" = "orange", "US" = "pink", "JP" = "lightblue", "CA" = "darkgreen","AU" = "brown","SI" = "yellow"))
...
fit102 <- lm(SI2y~.,data= components)
...
ggplot(residuals_data3, aes(x = date)) +
  ...
  labs(title = "Residuals for Each Country",
       x = "Date",
       y = "Residuals") +
  scale_color_manual(values = c("DE" = "blue", "FR" = "green", "ES" = "red", "US" = "orange"))
...
USfactor = data.frame(resUS, resAU, resCA)
as.matrix(USfactor)
...
summary(PCA_US)
components = as.data.frame(cbind(GF, EU, resUS, resJP, resGB, resCA))

fit123 <- lm(DE2y~.,data= components)
summary(fit123)$adj.r.squared

fit223 <- lm(FR2y~.,data= components)
summary(fit223)$adj.r.squared

fit323 <- lm(ES2y~.,data= components)
summary(fit323)$adj.r.squared
...
fit1023 <- lm(SI2y~.,data= components)
summary(fit1023)$adj.r.squared


#Construct series that cant be explained by the PCA1 (global Factor)
resDE3 = fit123$residuals
resFR3 = fit223$residuals
resES3 = fit323$residuals
...
ggplot(residuals_data3, aes(x = date)) +
  ...
  labs(title = "Residuals for Each Country",
       x = "Date",
       y = "Residuals") +
  scale_color_manual(values = c("DE" = "blue", "US" = "green", "GB" = "red"))

# Create a data frame for residuals
residuals_data4 <- data.frame(
  date = as.Date(dates),
  DE = resDE3,
  FR = resFR3,
  ES = resES3,
...
# Line chart of residuals for each country
ggplot(residuals_data4, aes(x = date)) +
  geom_line(aes(y = DE, color = "DE")) +
  geom_line(aes(y = FR, color = "FR")) +
  geom_line(aes(y = ES, color = "ES")) +
  geom_line(aes(y = US, color = "US")) +
...
US_coef <- contributions[contributions$predictor == "resUS", "Factor_contribution"]
...
GB_coef <- contributions[contributions$predictor == "resGB", "Factor_contribution"]
CA_coef <- contributions[contributions$predictor == "resCA", "Factor_contribution"]

GF_comp <- components[1]
US_comp <- components[3]
EU_comp <- components[2]
JP_comp <- components[4]
GB_comp <- components[5]
CA_comp <- components[6]
...
contribution_GB <- GB_coef * GB_comp
contribution_CA <- CA_coef * CA_comp
...
contributions_df <- data.frame(date = as.Date(dates),
                               yield = DE2y,
                               GF = In_coef + contribution_GF[, 1]- average_yield,
                               EU = contribution_EU[, 1]- average_yield,
                               US = contribution_US[, 1]- average_yield,
                               JP = contribution_JP[, 1]- average_yield,
                               GB = contribution_GB[, 1]- average_yield,
                               CA = contribution_CA[, 1]- average_yield,
                               idiosyncratic = augmented_data3$.resid)- average_yield
...
ggplot(contributions_df, aes(x = date, y = yield)) +
  ...
  geom_ribbon(aes(ymin = 0, ymax = GB, fill = "GB"), alpha = 0.5) +
  geom_ribbon(aes(ymin = 0, ymax = CA, fill = "CA"), alpha = 0.5) +
  geom_ribbon(aes(ymin = 0, ymax = idiosyncratic, fill = "Idio"), alpha = 0.5) +
  # Add more geom_ribbon layers for additional factors
  labs(title = "Global-European Factor Decomposition of FR 10Y Sovereign Nominal Yields",
       x = "date",
       y = "yield") +
  scale_fill_manual(values = c("GF" = "blue", "EU" = "green", "US" = "pink", "JP" = "lightblue", "GB" = "red", "CA" = "yellow", "Idio" = "orange")) +
  theme_minimal()
...
coefficients_table <- coef(summary(fit123))
contributions <- data.frame(
...
factor_contributions_DE = contributions[order(-contributions$Factor_contribution),]
...
coefficients_table <- coef(summary(fit62))
contributions6 <- data.frame(
  predictor = rownames(coefficients_table),
  Factor_contribution = coefficients_table[, 1]
)
print(contributions6)

# Sort the factor contributions in descending order
factor_contributions = contributions5[order(-contributions5$Factor_contribution),]

# Create a bar chart of the factor contributions
ggplot(contributions6, aes(x = predictor, y = Factor_contribution, fill = Factor_contribution)) +
  geom_bar(stat = "identity") +
  labs(title = "Factor Contributions to US2y",
       x = "Predictor",
       y = "Factor Contribution") +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5),
...
# Calculate the predicted values without the "GF" predictor
DE2y_GF = update(fit123, . ~ . -EU-US)
GF = predict(DE2y_GF)

contribution_GF = yhat_all - DE2y_GF$model

# Calculate the predicted values without the "GF" predictor
DE2y_GF = update(fit123, . ~ . -US)
GF = predict(DE2y_GF)
...
ggplot(contributions_df, aes(x = date, y = yield)) +
  ...
  geom_ribbon(aes(ymin = 0, ymax = idiosyncratic, fill = "Idio"), alpha = 0.5) +
  # Add more geom_ribbon layers for additional factors
  labs(title = "Global-European Factor Decomposition of FE 10Y Sovereign Nominal Yields",
       x = "date",
       y = "yield") +
  scale_fill_manual(values = c("GF" = "blue", "EU" = "green", "US" = "pink", "Idio" = "orange")) +
  theme_minimal()
...
aug=augmented_data1$.fitted - augmented_data1$.resid
...
contributions_df <- data.frame(date = as.Date(dates),
                               ...
                               US = augmented_data3$.fitted - augmented_data3$.resid -augmented_data2$.fitted + augmented_data2$.resid,
                               idiosyncratic = augmented_data3$.resid)
...
contributions_df <- data.frame(date = as.Date(dates),
                               ...
                               US = augmented_data3$.fitted -augmented_data2$.fitted,
                               idiosyncratic = augmented_data3$.resid)
