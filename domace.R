...
GTDEM <- read_excel("C:/Users/Karmen/Documents/magistrska/data1.xlsx", sheet = 2)
...
bond_yield_subset = as.matrix(bond_yield_data[,2:6])
...
# Create residuals_data again with the corrected date format
...
yield_data <- yield_data[complete.cases(yield_data$date), ]
...
ggplot(yield_data, aes(x = date)) +
  ...
  labs(title = "Donosnosti državnih obveznic",
       x = "Date",
       y = "Yield") +
...
# Remove rows with missing dates
residuals_data <- residuals_data[complete.cases(residuals_data$date), ]
...
# View the contributions for each predictor variable
contributions_df <- data.frame(GF = augmented_data$.fitted - augmented_data$.resid,
