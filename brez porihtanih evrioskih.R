...
GT2year <- read_excel("C:/Users/karzup7386/Desktop/mesec 2/PCA/10year2.xlsx", sheet = 6)
...
USfactor = data.frame(resUS, resAU, resCA, resGB)
as.matrix(USfactor)
...
summary(PCA_US)
components = as.data.frame(cbind(GF, resDE, resUS))
...
# Line chart of residuals for each country
ggplot(residuals_data5, aes(x = date)) +
  geom_line(aes(y = DE, color = "DE")) +
  geom_line(aes(y = FR, color = "FR")) +
  geom_line(aes(y = ES, color = "ES")) +
  geom_line(aes(y = IT, color = "IT")) +
...
ggplot(contributions_df, aes(x = date, y = yield)) +
  ...
  labs(title = "Global-European Factor Decomposition of DE 10Y Sovereign Nominal Yields",
       x = "date",
       y = "yield") +
  scale_fill_manual(values = c("GF" = "blue", "US" = "pink", "Idio" = "orange")) +
  theme_minimal()
